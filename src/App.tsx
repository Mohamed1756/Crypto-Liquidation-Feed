...
// Animation for stats change
const highlight = keyframes`
  0% { background-color: rgba(49, 151, 149, 0); }
  50% { background-color: rgba(49, 151, 149, 0.2); }
  100% { background-color: rgba(49, 151, 149, 0); }
`;
...
function App() {
  ...
  const [minLiquidationAlert, setMinLiquidationAlert] = useState(500000);
  ...
  const [themeAccent, setThemeAccent] = useState<'teal' | 'purple' | 'blue' | 'cyan'>('teal');
  ...
  const accentBgLight = `${themeAccent}.50`;
  const accentBgDark = `${themeAccent}.900`;
  const accentBg = useColorModeValue(accentBgLight, accentBgDark);
  const cardBg = useColorModeValue('white', 'gray.800');
  const mutedText = useColorModeValue('gray.600', 'gray.400');
  const hoverBg = useColorModeValue('gray.100', 'gray.700');
  const statsCardBg = useColorModeValue('white', 'gray.800');
  const pulseAnimation = `${pulse} 2s infinite`;
  const highlightAnimation = `${highlight} 2s ease`;
    ...
    return [
      {
        ...
        render: (
          <Stat>
            ...
            <Tooltip 
              label={`Buy: ${buyCount} vs Sell: ${sellCount}`}
              placement="top"
              hasArrow
            >
              <Box
                ...
                >
                  {stat.customContent ? (
                    ...
                    <Stat>
                      ...
                      {'detail' in stat && (
                        <StatHelpText>
                          {stat.detail as React.ReactNode}
                        </StatHelpText>
                      )}
                      {stat.max && (
                        <StatHelpText>
                          of {stat.max} available
                        </StatHelpText>
                      )}
                    </Stat>
                  )}
                    ...
                    <TabPanels>
                      ...
                      <TabPanel px={0} pt={4}>
                        ...
                        <Fade in={!isLoading} transition={{ enter: { duration: 0.5 } }}>
                          {isLoading ? (
                            <Skeleton height="300px" borderRadius="md" startColor={`${themeAccent}.100`} endColor={`${themeAccent}.500`} />
                          ) : (
                            <LiquidationTable 
    ...
    >
      <VStack spacing={6} align="stretch">
        <Flex align="center" justify="space-between">
          <Heading size="md" color={accentColor}>Advanced Analytics</Heading>
          <Tag colorScheme="yellow" size="sm">Coming Soon</Tag>
        </Flex>
